...
// ---------- версия схемы данных в телефоне ----------
// ДЗ/отметки (parket.state) и несохранённые правки (parket.pending) уже устойчивы к недостающим
// полям (см. normDoc()/emptyDoc() ниже) — это и есть их миграция, отдельный код не нужен.
// STORAGE_VERSION нужен на случай, если когда-нибудь СТРУКТУРА хранения изменится сильнее, чем
// «появилось новое поле» — тогда здесь будет что почистить. Пока просто отмечает версию и на
// каждом расхождении сбрасывает то, что и так безопасно перекачать заново (кеш расписания и
// журнал изменений), не трогая ДЗ и тем более несохранённые правки.
const STORAGE_VERSION = 1;
if (store.get('parket.schema') !== String(STORAGE_VERSION)) {
  Object.keys(PEOPLE).forEach((p) => { try { localStorage.removeItem('parket.cache.' + p); } catch (e) {} });
  try { localStorage.removeItem('parket.changes'); } catch (e) {}
  store.set('parket.schema', String(STORAGE_VERSION));
}
