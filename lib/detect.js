...
async function refreshChanges({ base, today, nowIso, profiles = ['me', 'her'], schedules }) {
  const detect = await Promise.all(profiles.map((profile) =>
    detectProfile({ base, profile, today, nowIso, schedules }).catch((e) => {
      const err = String((e && e.message) || e);
      console.error('detect: сверка расписания провалилась', profile, err); // иначе видно только по отсутствию новых записей в журнале
