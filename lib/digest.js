...
const { pairNo } = require('./slots');

const PEOPLE = require('../config/profiles.js');
const NAMES = { me: PEOPLE.me.name, her: PEOPLE.her.name };
const BUSY = { me: PEOPLE.me.busy, her: PEOPLE.her.busy };
const FREE = { me: PEOPLE.me.free, her: PEOPLE.her.free };
...
function expandCustom(id, c) {
  ...
  return dates.filter((d) => !skip.has(d)).map((d) => ({
    id: 'c:' + id + ':' + d, custom: true, num: 0, date: d, start: c.start, end: c.end,
...
function lessonLine(l, profile) {
  ...
  return `${no ? no + ' пара: ' : ''}${l.start}–${l.end} ${esc(l.subject)}${l.kind ? ' (' + esc(l.kind) + ')' : ''}${esc(room)}${sub}`;
}
...
function buildDigest({ who, date, now, schedules, docs, changes }) {
  ...
  if (changes && changes.length) {
    out += '<b>Изменилось в расписании</b>\n' + changes.slice(0, 6).map((e) => '• ' + esc(e.text)).join('\n');
    if (changes.length > 6) out += `\nи ещё ${changes.length - 6}`;
    out += '\n\n';
  }

  const tomorrow = mine.filter((l) => l.date === date);
  out += '<b>Твои пары</b>\n' + (tomorrow.length ? tomorrow.map((l) => lessonLine(l, who)).join('\n') : 'Пар нет.') + '\n\n';
